...
class BLOKHAUSRE_mls {
     ...
     * @var string
     */
    protected static $api_endpoint  = 'https://api.bridgedataoutput.com/api/v2/OData/miamire/Property/replication';
    protected static $api_endpoint2  = 'https://api.bridgedataoutput.com/api/v2/miamire/listings';
    ...
    public static function init() {
        static::$api_endpoint  = 'https://api.bridgedataoutput.com/api/v2/OData/miamire/Property/replication';
        static::$api_endpoint2  = 'https://api.bridgedataoutput.com/api/v2/miamire/listings';
    }
    ...
    public static function update($page = '') {
        ...
        $limit_query = ( isset($limit_query) || $limit_query!='' ) ? $limit_query : '200' ;
        ...
        // $date_properties_yesterday = str_replace('/', '-', $date_properties_yesterday);        
        $date_properties_yesterday = $date_properties_yesterday.'T00:00:00Z';
        WP_CLI::log( 'Date properties yesterday: '.$date_properties_yesterday);
        $array_neighborhood = self::getListNeighnorhood();
        for ($i=0; $i < count($array_neighborhood); $i++) { 
            if ($array_neighborhood[$i]!='69' && $array_neighborhood[$i]!='76') {
                $array_subdivisiones = self::getSubdivisionListByNeighnorhood($array_neighborhood[$i]);
                if (isset($array_subdivisiones)) {
                    ...
                    $queries      = array(
                        ...
                        'limit'         => $limit_query,
                        'MlsStatus'     => $mlsStatus,
                         // comment the next line to get all properties by date from API, or use BridgeModificationTimestamp to get more properties
                        'MajorChangeTimestamp.gt' => $date_properties_yesterday,
                    );

                    for ($j=0; $j < count($array_subdivisiones); $j++) { 
                        $queries += [ "or[$j][SubdivisionName][prefix]" => $array_subdivisiones[$j] ];
                    }
    ...
    public static function getPropertyOfMLS($queries, $api_args, $page, $limit_query){
        ...
        $property_data = add_query_arg( $queries, static::$api_endpoint2 );
        ...
        if ( $coderesponse == 200 ) {
            ...
            $pos = strpos($next_link,"=");
            $lenght = strlen($next_link);

            $new_link = substr($next_link,$pos+1,$lenght);
            $pos2 = strpos($new_link,"=");
            $lenght2 = strlen($new_link);

            $next_property_link = substr($new_link,$pos2+1,$lenght2);
            WP_CLI::log( 'Next Link: '.$next_property_link );
            */
            ...
            for($i = 0; $i < count($result->bundle); $i++){
                ...
                //$existing_posts = get_posts( array('post_type' => 'propertymls', 'numberposts' => -1) );                   
                //foreach($existing_posts as $post) {             
                    //$propertyid = get_post_meta( $post->ID, 'mls_property_id', true );
                    //array_push($api_ids, $propertyid);
                //}

                $existingPost = self::get_post_by_meta( array( 'meta_key' => 'mls_property_id', 'meta_value' => $ListingId ) );

                //$action = !in_array($ListingId, $api_ids ) ? 'insert' : 'update';
                ...
                if ( $action == 'update' ) {
                    ...
                    WP_CLI::log( $message );
                } elseif ( $action == 'insert' && isset($area) && $area!='' ) {  
                    $new_post = array(
                        'post_type'     => 'propertymls',
                        'post_title'    => $title,
                        'post_status'   => 'publish',
            ...
            $nextPage = $page+1;
            if ( $nextPage <= $totalPages && $nextPage <= 30 ) {
    ...
    public static function get_Area($SubdivisionName){
        ...
        $parent_terms = get_terms( $taxonomyName, array( 'parent' => $number = 76, 'orderby' => 'slug', 'hide_empty' => false ) );   
        $area = '';
    ...
    public static function getSubdivisionListByNeighnorhood($neighborhoodID){
        $taxonomyName = 'property_area';
            ...
            foreach ( $parent_terms as $pterm ) {
                ...
                if ($repeater_value) { 
                    for ($i=0; $i<$count; $i++) {
                        $meta_key = $repeater.'_'.$i.'_'.'subdivision_items';
                        $sub_field_value = get_term_meta($taxonomy_id, $meta_key, true);
                        if ($taxonomy_id == $neighborhoodID) {
                            $subdivisionList[] = $sub_field_value;
                        }
                    }
                }
                $termchildren = get_term_children( $taxonomy_id, $taxonomyName );
                if (!empty($termchildren)) {
                    foreach ( $termchildren as $child ) {
                        $term = get_term_by( 'id', $child, $taxonomyName );
                        $taxonomy_id = $term->term_id;            
    ...
    public static function getListNeighnorhood(){
        $taxonomyName = 'property_area';
        $parent_terms = get_terms( $taxonomyName, array( 'parent' => 0, 'orderby' => 'slug', 'hide_empty' => false ) );   
        $area = '';
