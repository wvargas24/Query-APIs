...
class BLOKHAUSRE_Condos_daily_mls {
    ...
    public static function update($page = '') {
        ...
        $array_condos = self::getListCondos();
        for ($i=0; $i < count($array_condos)-1; $i++) { 
            //WP_CLI::log( 'Condo ID: '.$array_condos[$i] );
            $addressCondo = self::getAddressByCondoId($array_condos[$i]);
            //WP_CLI::log( 'Condo address: '.$addressCondo );
            if (isset($addressCondo) && $addressCondo != '') {
                ...
                $queries      = array(
                    ...
                    'order'         => 'desc',
                    //'MlsStatus'     => $mlsStatus,
                    "or[1][MlsStatus][prefix]" => 'Active',
                    "or[2][MlsStatus][prefix]" => 'Pending',
                    //'$filter'       => 'year(ModificationTimestamp) eq 2021',
                    'MajorChangeTimestamp.gt' => $date_properties_yesterday,
                    "or[1][PropertyType][prefix]" => 'Residential',
                    "or[2][PropertyType][prefix]" => 'Residential%20Lease',
                    //"PropertyType"  => 'Residential%20Lease',
                    "or[1][StructureType][prefix]" => 'Condo',
                    "or[2][StructureType][prefix]" => 'Townhouse',
                    "or[1][PropertySubType][prefix]" => 'Condominium',
                    "or[2][PropertySubType][prefix]" => 'Townhouse',
                    "UnparsedAddress" => $addressCondo, 
                    //"or[0][ListingId][prefix]" => 'A11042557',
                    //"or[1][ListingId][prefix]" => 'A11129228',
                    //"or[2][ListingId][prefix]" => 'A11132226',
                    //"UnparsedAddress" => '3400 SW 27th Ave',
                    //'MlsStatus'     => 'Pending',
                );
            ...
            foreach ( $parent_terms as $pterm ) {
                ...
                if ($repeater_value) {
                    for ($i=0; $i<$count; $i++) {
                        ...
                        $sub_field_value = get_term_meta($taxonomy_idParent, $meta_key, true);
                        if ($sub_field_value!='') {
    ...
    public static function getAddressByCondoId($condoID){
        //WP_CLI::log( 'condoID: '.$condoID );
        $addressCondo = '';
        $args = array(
            'post__in'         => array( $condoID ),
            'posts_per_page'   => 1,
            'post_type'        => 'condos',
            'post_status'      => 'publish',
        );
    ...
    public static function getListCondos(){
        $condosList = array();
        $args = array(
            'posts_per_page'   => -1,
            'post_type'        => 'condos',
