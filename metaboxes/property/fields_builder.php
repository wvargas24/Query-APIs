 ...
 * @return array
 */
function blokhausre_field_builder_metabox_fields( $metabox_fields ) {
	...
	if( class_exists('Houzez_Fields_Builder') ) {
		...
		if( !empty($fields_array) ) {
			...
			foreach ($fields_array as $field) {
                ...
                $field_id = blokhausre_clean_20($field->field_id);

                $field_title = blokhausre_wpml_translate_single_string($field_title);
                $field_placeholder = blokhausre_wpml_translate_single_string($field_placeholder);
                ...
                if($field_type == 'select') {
                    ...
                    if(!empty($options)) {
                        foreach ($options as $key => $val) {
                            ...
                            $select_options = blokhausre_wpml_translate_single_string($val);
...
add_filter( 'blokhausre_property_metabox_fields', 'blokhausre_field_builder_metabox_fields', 15 );
